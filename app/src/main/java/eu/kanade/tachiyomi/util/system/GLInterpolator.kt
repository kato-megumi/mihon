...
object GLInterpolator {
    ...
    fun scale(
        ...
        return try {
            ...
            // GL resource holders; deleted in finally so an exception mid-render can't leak them
            val framebuffer = IntArray(1)
            val outputTexture = IntArray(1)
            val inputTexture = IntArray(1)
            var framebufferCreated = false
            var outputTextureCreated = false
            var inputTextureCreated = false
            try {
                ...
                // Always release GL resources, even if rendering/readback threw
                if (inputTextureCreated) GLES20.glDeleteTextures(1, inputTexture, 0)
                if (outputTextureCreated) GLES20.glDeleteTextures(1, outputTexture, 0)
                if (framebufferCreated) GLES20.glDeleteFramebuffers(1, framebuffer, 0)
    ...
    fun release() {
        ...
        try {
            ...
            // Clear handles so a stale surface/context can't be reused after release
